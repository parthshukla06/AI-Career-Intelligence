...
import { analyzeResume, getResumeAnalysisModel, ResumeAnalysisError } from "../services/resumeAnalyzer.service";
import { generateCandidateEmbedding } from "../services/embedding.service";

export const uploadResume = async (req: Request, res: Response): Promise<void> => {
  if (!req.file) {
    res.status(400).json({ success: false, message: "A PDF file is required in the resume field" });
    return;
  }
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Unable to process the uploaded resume";
  ...
  try {
    ...
    if (!(error instanceof ResumeAnalysisError)) {
      res.status(502).json({ success: false, message: "Resume analysis could not be completed. Please try again later." });
      return;
    }
    ...
    try {
      ...
      console.error("Failed to save resume analysis failure:", saveError);
      res.status(500).json({ success: false, message: "Unable to save resume processing status" });
      return;
    }
  ...
  try {
    candidateEmbedding = await generateCandidateEmbedding(candidateProfile);
  } catch (error) {
    console.error("Candidate embedding generation failed:", error instanceof Error ? error.message : "Unknown error");
  }
  ...
  try {
    ...
    res.status(201).json({
      ...
      data: { id: resume._id, originalFilename: resume.originalFilename, candidateProfile: resume.candidateProfile },
    });
  } catch (error) {
    console.error("Failed to save analyzed resume:", error);
    res.status(500).json({ success: false, message: "Unable to save analyzed resume" });
  }
};
