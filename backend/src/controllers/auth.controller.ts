  ...
  if (
    ...
    res
      ...
      .json({
        success: false,
        message:
          "name, valid email, and password of 8-200 characters are required",
      });
    return;
  }
  try {
    ...
    res
      ...
      .json({
        success: true,
        data: { user: publicUser(user), token: issueToken(String(user._id)) },
      });
  } catch (error) {
    ...
    res
      .status(500)
      .json({ success: false, message: "Unable to create account" });
  }
};
